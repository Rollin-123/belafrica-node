...
export class SupabaseService {
  ...
  // ✅ TROUVER un utilisateur par téléphone
   async findUserByPhone(phoneNumber: string): Promise<any> {
    try {
      const { data, error } = await this.client
        .from('users')
        .select('*')
  ...
  async findUserByPseudo(pseudo: string, excludeUserId?: string): Promise<any> {
    try {
 ...
 async hasValidOTP(phoneNumber: string): Promise<boolean> {
  ...
  async promoteToAdmin(userId: string, permissions: string[]): Promise<boolean> {
    try {
      const { error } = await this.client
        .from('users')
        .update({
  ...
  async getCommunityPosts(community: string, visibility: 'national' | 'international' = 'national'): Promise<any[]> {
    try {
      const { data, error } = await this.client
        .from('posts')
        .select(`
  ...
  async getInternationalPosts(): Promise<any[]> {
    try {
      const { data, error } = await this.client
        .from('posts')
        .select(`
  ...
  async createAdminRequest(requestData: any): Promise<any> {
    try {
      const { data, error } = await this.client
        .from('admin_requests')
        .insert([{
  ...
  async generateAdminCode(codeData: AdminCodeData): Promise<any> {
    try {
  ...
  async validateAdminCode(code: string, community: string): Promise<boolean> {
    try {
      const { data, error } = await this.client
        .from('admin_codes')
        .select('*')
  ...
  async markAdminCodeAsUsed(code: string, userId: string): Promise<void> {
    try {
      const { error } = await this.client
        .from('admin_codes')
        .update({
  ...
  async updateAdminRequestStatus(requestId: string, status: string): Promise<boolean> {
    try {
      const { error } = await this.client
        .from('admin_requests')
        .update({
  ...
  async getGeneratedAdminCodes(): Promise<any[]> {
    try {
      const { data, error } = await this.client
        .from('admin_codes')
        .select(`
  ...
  async getCommunityUsers(community: string, excludeUserId?: string): Promise<any[]> {
    try {
      let query = this.client
        .from('users')
  ...
  async addConversationParticipant(conversationId: string, userId: string, isAdmin: boolean = false): Promise<void> {
    try {
      const { error } = await this.client
        .from('conversation_participants')
        .insert([{
  ...
  async getCommunityGroup(communityId: string): Promise<any> {
    try {
      const { data, error } = await this.client
        .from('conversations')
        .select(`
  ...
  async getPrivateConversation(userId1: string, userId2: string): Promise<any> {
    try {
      ...
      const validConversation = data?.find((conv: any) => {
